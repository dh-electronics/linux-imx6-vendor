...
{
	return !gpio_get_value(tsc2004gpio);
}
/*
 * Translate OpenFirmware node properties into platform_data
 */
static int tsc2004_get_devtree_pdata(struct device *dev,
			    struct tsc2004_platform_data *pdata)
{
	struct device_node *node, *pp = dev->of_node;
	int ret;
	int gpio;
	const u32 *reg;
	int len;
	...
	reg = of_get_property(pp, "tsc_model", &len);
	pdata->model = be32_to_cpup(reg);
	pdata->x_plate_ohms	= 180;

	gpio = of_get_named_gpio(pp, "irq-gpios", 0);
	if (!gpio_is_valid(gpio))
		return -ENODEV;

	tsc2004gpio = gpio;
	ret = gpio_request(gpio, "tsc2004_irq");
	if (ret < 0) {
		dev_err(dev, "request gpio failed, cannot wake up controller: %d\n", ret);
		return ret;
	}
	...
	pdata->get_pendown_state = tsc2004_get_pendown_state;
...
static int tsc2004_get_devtree_pdata(struct device *dev, struct tsc2004 *ts,
			    struct tsc2004_platform_data *altp)
{
	return -ENODEV;
}
#endif
...
static struct of_device_id tsc2004_dt_ids[] = {
...
static struct i2c_driver tsc2004_driver = {
	.driver = {
		.owner	= THIS_MODULE,
		.name	= "tsc2004",
		.of_match_table	= of_match_ptr(tsc2004_dt_ids),
	},
